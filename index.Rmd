...
plot_custom_boxplot(school_data_factor, "attendance_rate",
...
plot_custom_boxplot(school_data_factor, "reduced_share",
...
plot_custom_boxplot(school_data_factor, "free_share",
                    "Students Eligible for Free-priced Lunch (%)",
                    "Students Receiving Free-priced Meals") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 0.5)) +
  annotate("text", x = 8, y = 0, label = "*2021 Omitted", color = "red3")

plot_custom_boxplot(school_data_factor, "homeless_share",
...
```{r, message=FALSE, warning=FALSE}
plot_relationship("reduced_share", "Reduced-price Lunch Eligibility") +
  scale_x_continuous(labels = scales::percent)

plot_relationship("free_share", "Free-price Lunch Eligibility") +
  scale_x_continuous(labels = scales::percent)

plot_relationship("tec", "Cost Per Student")

plot_relationship("enrollment", "District Enrollment")
```

## Multiple Regression

Breusch-Pagan Test: determines whether the panel effect exists.

Hausman Test: determines whether the error is correlated with the explanatory variables.
```{r, message=FALSE, warning=FALSE}
school_data_alt <- school_data |> 
  filter(year != 2020)

ols_model <- plm(attendance_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
...
fe_model <- plm(attendance_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
                data = school_data_alt, 
                index = c("dist_code", "year"), 
                model = "within") 

re_model <- plm(attendance_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
                data = school_data_alt, 
                index = c("dist_code", "year"), 
                model = "random")
...
p_values <- tibble(
  Test = c("Breusch-Pagan", "Hausman"),
  "P-value" = c(bptest$p.value, hausman_test$p.value)
)

kable(p_values, format = "markdown", digits = 5) |> 
  kable_styling(full_width = FALSE, position = "center")
```


```{r, message=FALSE, warning=FALSE, results='hide'}
attendance_ols <- plm(attendance_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
                        data = school_data_alt, 
                        index = c("dist_code", "year"), 
                        model = "pooling")
graduation_ols <- plm(graduation_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
                        data = school_data_alt, 
                        index = c("dist_code", "year"), 
                        model = "pooling")
act_ols <- plm(act_composite ~ reduced_share + free_share + log(enrollment) + log(tec), 
                        data = school_data_alt, 
                        index = c("dist_code", "year"), 
                        model = "pooling")


attendance_fixed <- plm(attendance_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
                        data = school_data_alt, 
                        index = c("dist_code", "year"), 
                        model = "within")
graduation_fixed <- plm(graduation_rate ~ reduced_share + free_share + log(enrollment) + log(tec), 
                        data = school_data_alt, 
                        index = c("dist_code", "year"), 
                        model = "within")
act_fixed <- plm(act_composite ~ reduced_share + free_share + log(enrollment) + log(tec), 
                        data = school_data_alt, 
                        index = c("dist_code", "year"), 
                        model = "within")


attendance_results <- list(attendance_ols, attendance_fixed, graduation_ols, graduation_fixed, act_ols, act_fixed)

stargazer(attendance_results,
          type = "text",
          ...
          column.labels = c("(OLS)", "(Fixed)", "(OLS)", "(Fixed)", "(OLS)", "(Fixed)"),
          model.names = FALSE,
          covariate.labels = c(
